...
big_df_result = pd.concat(
    file_dictionary_result.values(), axis=1, keys=file_dictionary_result.keys()
)
big_df_before = pd.concat(
    file_dictionary_before.values(), axis=1, keys=file_dictionary_before.keys()
)
big_df_after = pd.concat(
    file_dictionary_after.values(), axis=1, keys=file_dictionary_after.keys()
)
...
# now let's output graphs of the distributions of these summary statistics
# first transpose dataframes
# big_df_result_mean_t = big_df_result_mean.transpose()
# big_df_before_mean_t = big_df_before_mean.transpose()
# big_df_after_mean_t = big_df_after_mean.transpose()
